...
function calcTotals(data: PnlData): PnlTotals {
...
function DeltaCell({ current, previous }: DeltaCellProps) {
...
export default function PnlReportPage() {
  ...
  const [currentData, setCurrentData] = useState<PnlData | null>(null)
  const [prevData, setPrevData] = useState<PnlData | null>(null)
  ...
  if (loading || !current || !prev || !currentData || !prevData) {
    return (
            ...
            <tbody>
              ...
              {[
                ...
                <tr key={item.key}>
                  ...
                  <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(currentData.opex[item.key as keyof typeof currentData.opex])}</td>
                  {multiPeriod && <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(prevData.opex[item.key as keyof typeof prevData.opex])}</td>}
                  {multiPeriod && <DeltaCell current={currentData.opex[item.key as keyof typeof currentData.opex]} previous={prevData.opex[item.key as keyof typeof prevData.opex]} />}
