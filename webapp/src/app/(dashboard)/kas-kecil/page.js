...
function KasKecilPageContent() {
  ...
  const [categories, setCategories] = useState(PETTY_CASH_CATEGORIES)

  // --- Filter states ---
  const [filterMonth, setFilterMonth] = useState(new Date().getMonth())
  const [filterYear, setFilterYear] = useState(new Date().getFullYear())
  const [filterType, setFilterType] = useState('all') // 'all' | 'expense' | 'topup'

  // --- Transactions data ---
  const [transactions, setTransactions] = useState([])
  const [selectedReceipt, setSelectedReceipt] = useState(null) // Modal popup for receipt image

  // --- Form states ---
  const [inputDate, setInputDate] = useState(new Date().toISOString().split('T')[0])
  const [inputBranchId, setInputBranchId] = useState('')
  const [inputType, setInputType] = useState('expense') // 'expense' | 'topup'
  ...
  const loadTransactions = async () => {
    ...
    if (supabaseActive) {
      try {
        ...
        setTransactions(mapped.sort((a, b) => b.tanggal - a.tanggal))
      } catch (err) {
        console.error('Failed to load transactions from Supabase:', err)
        addToast({ title: 'Error', message: 'Gagal memuat log kas kecil dari server.', type: 'error' })
      } finally {
        setLoading(false)
      }
    } else {
  ...
  const handleSaveTransaction = async (e) => {
    ...
    if (!inputBranchId) {
      addToast({ title: 'Peringatan', message: 'Silakan pilih cabang.', type: 'warning' })
      return
    }

    if (inputType === 'expense' && !inputCategoryId) {
      addToast({ title: 'Peringatan', message: 'Silakan pilih kategori pengeluaran.', type: 'warning' })
      return
    }

    const amountNum = parseFloat(inputAmount) || 0
    if (amountNum <= 0) {
      addToast({ title: 'Peringatan', message: 'Nominal transaksi harus lebih besar dari 0.', type: 'warning' })
      return
    }
    ...
    if (supabaseActive) {
      try {
        ...
        if (receiptFile) {
          // In real supabase we would upload to bucket
          // const { data, error } = await supabase.storage.from('receipts').upload(path, file)
          // For integration, we use the local base64 preview or a mock URL
        ...
        const { error } = await supabase
          ...
          .insert({
            ...
            category_id: inputType === 'topup' ? '00000000-0000-0000-0000-000000000000' : inputCategoryId, // dummy or custom topup UUID
        ...
        addToast({ title: 'Sukses', message: 'Transaksi kas kecil berhasil disimpan.', type: 'success' })
        resetForm()
        setActiveTab('list')
        loadTransactions()
      } catch (err) {
        console.error('Failed to save petty cash to Supabase:', err)
        addToast({ title: 'Error', message: 'Gagal menyimpan transaksi kas kecil ke server.', type: 'error' })
      } finally {
        setLoading(false)
      }
    } else {
      ...
      try {
        ...
        const localPC = JSON.parse(localPCStr)
        ...
        addToast({ title: 'Sukses (Demo)', message: `Transaksi ${inputType === 'topup' ? 'Top-up' : 'Pengeluaran'} Rp ${amountNum.toLocaleString()} berhasil disimpan secara lokal.`, type: 'success' })
        resetForm()
        setActiveTab('list')
        loadTransactions()
      } catch (err) {
        console.error('Failed to save petty cash to localStorage:', err)
        addToast({ title: 'Error', message: 'Gagal menyimpan secara lokal.', type: 'error' })
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleDeleteTransaction = async (id) => {
    ...
    if (supabaseActive) {
      try {
        ...
        addToast({ title: 'Sukses', message: 'Transaksi berhasil dihapus.', type: 'success' })
        loadTransactions()
      } catch (err) {
        console.error('Failed to delete transaction:', err)
        addToast({ title: 'Error', message: 'Gagal menghapus transaksi.', type: 'error' })
      } finally {
        setLoading(false)
      }
    } else {
      ...
      try {
        const localPCStr = localStorage.getItem('raja-aksesoris-petty-cash')
        if (localPCStr) {
          const parsed = JSON.parse(localPCStr)
          const filtered = parsed.filter(t => t.id !== id)
          localStorage.setItem('raja-aksesoris-petty-cash', JSON.stringify(filtered))
        }
        addToast({ title: 'Sukses (Demo)', message: 'Transaksi berhasil dihapus.', type: 'success' })
        loadTransactions()
      } catch (err) {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    // Filter by branch
    if (selectedBranch) {
      list = list.filter(item => String(item.branchId) === String(selectedBranch))
    }
  ...
  const currentBalance = useMemo(() => {
    // We calculate the running balance for the currently selected branch
    // If "Semua Cabang" is selected, we show total consolidated balance
    let list = transactions
    if (selectedBranch) {
      list = transactions.filter(t => String(t.branchId) === String(selectedBranch))
    }
            ...
            <div className="stat-card" style={{ flex: 1 }}>
              <div className="stat-card-header">
                <span className="stat-card-label">
                  Saldo Kas Kecil {selectedBranch ? `— Cabang ${branches.find(b => b.id === selectedBranch)?.name}` : '— Semua Cabang'}
                </span>
                ...
                <div style={{ width: '120px', height: '120px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f5f5f7', position: 'relative' }}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setReceiptFile(null)
                      setReceiptPreview('')
                      document.getElementById('pc-receipt').value = ''
                    }}
                  >
                    &times;
                  </button>
