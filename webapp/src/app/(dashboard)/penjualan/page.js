...
function PenjualanPageContent() {
  ...
  const [expandedRow, setExpandedRow] = useState(null) // date-branch key

  // --- Cash Deposit states ---
  const [unDepositedCash, setUnDepositedCash] = useState(0)
  ...
  const [salesInputs, setSalesInputs] = useState({}) // { [methodId]: amount }
  const [inputRetur, setInputRetur] = useState('')
      ...
      try {
        ...
        if (localSales) {
          ...
          const filteredSales = activeBranchId
            ? parsedSales.filter(s => String(s.branchId) === String(activeBranchId))
            : parsedSales

          filteredSales.forEach(s => {
            // Cash payment method ID is 'pay-1'
            const cashAmount = s.sales?.['pay-1'] || s.sales?.['Cash'] || 0
            totalCashSales += parseFloat(cashAmount) || 0
          })
        }

        const localDeposits = localStorage.getItem('raja-aksesoris-cash-deposits')
        if (localDeposits) {
          const parsedDeposits = JSON.parse(localDeposits)
          const filteredDeposits = activeBranchId
            ? parsedDeposits.filter(d => String(d.branchId) === String(activeBranchId))
            : parsedDeposits
    ...
    if (supabaseActive) {
      try {
        ...
        data.forEach((row) => {
          ...
          const mdrRate = row.payment_methods?.mdr_rate || 0
          ...
          grouped[key].breakdown.push({
            methodId: row.payment_methods?.id,
            methodName,
            amount,
            mdr,
            expected: amount - mdr
        ...
        const list = Object.values(grouped).sort((a, b) => b.date - a.date)
        setSalesList(list)
      } catch (err) {
        console.error('Failed to load sales list from Supabase:', err)
        addToast({ title: 'Error', message: 'Gagal memuat data penjualan dari server.', type: 'error' })
      } finally {
        setLoading(false)
      }
    } else {
      ...
      try {
        ...
        if (localSales) {
          ...
          const mapped = filtered.map(item => {
            ...
            const breakdown = Object.entries(item.sales).map(([methodId, amount]) => {
              const method = paymentMethods.find(m => String(m.id) === String(methodId))
              const name = method ? method.name : 'Unknown'
              const rate = method ? parseFloat(method.mdrRate) : 0
          ...
          setSalesList(mapped.sort((a, b) => b.date - a.date))
        } else {
          setSalesList([])
        }
      } catch (err) {
  ...
  const handleSaveSales = async (e) => {
    ...
    if (!inputBranchId) {
      addToast({ title: 'Peringatan', message: 'Silakan pilih cabang terlebih dahulu.', type: 'warning' })
      return
    }

    const branchObj = branches.find(b => String(b.id) === String(inputBranchId))
    const branchName = branchObj ? branchObj.name : 'Unknown'

    // Verify at least one input is non-zero
    const hasValues = Object.values(salesInputs).some(val => val > 0)
    if (!hasValues) {
      addToast({ title: 'Peringatan', message: 'Silakan masukkan nominal penjualan minimal untuk satu metode pembayaran.', type: 'warning' })
      return
    }
    ...
    if (supabaseActive) {
      try {
        ...
        for (const method of paymentMethods) {
          ...
          if (amount >= 0) {
            ...
            insertPromises.push(
              supabase.from('daily_sales').upsert({
            ...
            const mdrVal = amount * parseFloat(method.mdrRate)
            reconPromises.push(
        ...
        addToast({ title: 'Sukses', message: `Data penjualan untuk ${branchName} tanggal ${formatDate(new Date(inputDate))} berhasil disimpan.`, type: 'success' })
        setSalesInputs({})
        setInputRetur('')
        setInputNotes('')
        setActiveTab('list')
        ...
        addToast({ title: 'Error', message: 'Gagal menyimpan data penjualan ke server.', type: 'error' })
      } finally {
        setLoading(false)
      }
    } else {
      ...
      try {
        ...
        const updatedSales = localSales.filter(item => `${item.dateStr}-${item.branchId}` !== key)
        ...
        localRecons = localRecons.filter(r => !(r.branchId === inputBranchId && r.date === inputDate))
        ...
        addToast({ title: 'Sukses (Demo)', message: `Data penjualan untuk ${branchName} tanggal ${formatDate(new Date(inputDate))} disimpan lokal.`, type: 'success' })
        setSalesInputs({})
        setInputRetur('')
        setInputNotes('')
        setActiveTab('list')
        ...
        addToast({ title: 'Error', message: 'Gagal menyimpan data secara lokal.', type: 'error' })
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSaveDeposit = (e) => {
    ...
    if (amountNum <= 0) {
      addToast({ title: 'Peringatan', message: 'Nominal setoran harus lebih besar dari 0.', type: 'warning' })
      return
    }
    ...
    const branchObj = branches.find(b => String(b.id) === String(activeBranchId))
    const branchName = branchObj ? branchObj.name : 'Unknown'
    ...
    if (supabaseActive) {
      ...
      try {
        ...
        const localDeposits = JSON.parse(localDepositsStr)
        ...
        addToast({ title: 'Sukses (Demo)', message: `Setoran tunai Rp ${amountNum.toLocaleString()} berhasil disimpan secara lokal.`, type: 'success' })
        ...
        addToast({ title: 'Error', message: 'Gagal menyimpan setoran.', type: 'error' })
      }
    }
  }
  ...
  const handleDeleteSales = async (item) => {
    ...
    if (supabaseActive) {
      try {
        ...
        addToast({ title: 'Sukses', message: 'Data penjualan berhasil dihapus.', type: 'success' })
        loadSalesList()
      } catch (err) {
      ...
      try {
        const localSalesStr = localStorage.getItem('raja-aksesoris-daily-sales')
        if (localSalesStr) {
          const parsed = JSON.parse(localSalesStr)
          const filtered = parsed.filter(i => !(i.dateStr === dateStr && i.branchId === item.branchId))
          localStorage.setItem('raja-aksesoris-daily-sales', JSON.stringify(filtered))
        }

        const localReconStr = localStorage.getItem('raja-aksesoris-reconciliations')
        if (localReconStr) {
          const parsed = JSON.parse(localReconStr)
          const filtered = parsed.filter(i => !(i.date === dateStr && i.branchId === item.branchId))
          localStorage.setItem('raja-aksesoris-reconciliations', JSON.stringify(filtered))
        }

        addToast({ title: 'Sukses (Demo)', message: 'Data penjualan berhasil dihapus dari penyimpanan lokal.', type: 'success' })
        loadSalesList()
      } catch (err) {
  ...
  const filteredSalesList = useMemo(() => {
    ...
    return salesList.filter(item => String(item.branchId) === String(branchId))
                ...
                <form onSubmit={handleSaveDeposit}>
                  ...
                  <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                    <div className="input-group">
                      ...
                      <input id="dep-branch" className="input" value={branchName || 'Semua Cabang (Pilih cabang di header)'} disabled style={{ opacity: 0.7 }} />
                    </div>
              ...
              <div className="input-group">
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    value={inputRetur}
                    onChange={(e) => setInputRetur(e.target.value)}
                  />
                </div>
              </div>
            </div>
