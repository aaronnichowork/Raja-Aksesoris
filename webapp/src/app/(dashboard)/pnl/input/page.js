...
// ---------------------------------------------------------------------------
function CurrencyInput({ id, label, value, onChange, readOnly, source, helper }) {
    ...
    <div className="input-group">
      ...
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          value={focused ? (value === 0 ? '' : value || '') : displayValue.replace('Rp ', '')}
...
export default function PnlInputPage() {
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    try {
      await savePnlData(month, year, branch, data)
      setSaved(true)
